...
// POST /api/admin-mgmt/send-login-otp - Send OTP for admin login (passwordless)
// Only allowed email: exchangebluestone5@gmail.com
const ALLOWED_ADMIN_EMAIL = 'exchangebluestone5@gmail.com'
...
router.post('/send-login-otp', async (req, res) => {
  try {
    ...
    // Only allow specific email for admin login
    ...
    if (!admin) {
      ...
      admin = await Admin.create({
        ...
        password: await bcrypt.hash('temp_password_not_used', 10),
        firstName: 'Super',
        lastName: 'Admin',
        role: 'SUPER_ADMIN',
        status: 'ACTIVE',
        urlSlug: 'super-admin-' + Date.now()
      })
...
// POST /api/admin-mgmt/verify-login-otp - Verify OTP and complete admin login
router.post('/verify-login-otp', async (req, res) => {
  try {
    ...
    const otpRecord = await OTP.findOne({ email: email.toLowerCase(), otp, purpose: 'admin_login' })
    if (!otpRecord) {
