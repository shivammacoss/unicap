...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    if (emailResult.success) {
      ...
      console.error('Email settings check - SMTP enabled:', settings?.smtpEnabled, 'OTP enabled:', settings?.otpVerificationEnabled, 'SMTP Host:', settings?.smtpHost ? 'configured' : 'missing')
      
      // Return error instead of silently proceeding without OTP
      res.status(500).json({ success: false, otpRequired: true, message: `Failed to send OTP: ${emailResult.message}` })
    }
  } catch (error) {
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    const baseUrl = process.env.CORS_ORIGIN || 'https://bluestoneexchange.com'
...
router.post('/signup', async (req, res) => {
  try {
    const { firstName, email, phone, countryCode, address, password, adminSlug, referralCode, otpVerified } = req.body
    ...
    // Send welcome email only if OTP verification is disabled
    ...
    if (smtpEnabled) {
      ...
      try {
        await sendTemplateEmail('password_reset', email, {
          ...
          otp,
          expiryMinutes,
          platformName: 'BlueStone',
          supportEmail: emailSettings?.fromEmail || 'support@BlueStone.com',
          year: new Date().getFullYear().toString()
        })
