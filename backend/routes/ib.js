...
router.post('/reapply', async (req, res) => {
  try {
    ...
    // Find existing rejected IB application
    const existingIB = await IBUser.findOne({ userId })
    if (!existingIB) {
      return res.status(404).json({ message: 'No previous application found' })
    }
    ...
    existingIB.rejectedBy = null
    existingIB.rejectedAt = null
    existingIB.rejectionReason = null
    await existingIB.save()

    // Also update user's ibStatus
    await User.findByIdAndUpdate(userId, { ibStatus: 'PENDING' })
    ...
    console.error('Error re-applying as IB:', error)
    res.status(500).json({ message: 'Error submitting re-application', error: error.message })
  }
})
