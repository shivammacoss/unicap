...
const BannerSlider = ({ isDarkMode = true, bannerType = 'hero' }) => {
  ...
  const fetchBanners = async () => {
    try {
      const res = await fetch(`${API_URL}/banners/active?type=${bannerType}`)
  ...
  // Get proper image URL - handle both relative and absolute URLs
  const getImageUrl = (imageUrl) => {
    if (!imageUrl) return 'https://via.placeholder.com/1200x400?text=Banner'
