...
const AdminForexCharges = () => {
  ...
  const [instruments, setInstruments] = useState([
    ...
    { symbol: 'XPTUSD', name: 'Platinum', category: 'Metals' },
    { symbol: 'XPDUSD', name: 'Palladium', category: 'Metals' },
    { symbol: 'XAUEUR', name: 'Gold/EUR', category: 'Metals' },
    { symbol: 'XAUAUD', name: 'Gold/AUD', category: 'Metals' },
    { symbol: 'XAUGBP', name: 'Gold/GBP', category: 'Metals' },
    { symbol: 'XAUCHF', name: 'Gold/CHF', category: 'Metals' },
    { symbol: 'XAUJPY', name: 'Gold/JPY', category: 'Metals' },
    { symbol: 'XAGEUR', name: 'Silver/EUR', category: 'Metals' },
    // Crypto (15)
    ...
    { symbol: 'UK100', name: 'UK 100', category: 'Indices' },
    { symbol: 'GER40', name: 'Germany 40', category: 'Indices' },
    { symbol: 'FRA40', name: 'France 40', category: 'Indices' },
    { symbol: 'JPN225', name: 'Japan 225', category: 'Indices' },
    { symbol: 'AUS200', name: 'Australia 200', category: 'Indices' },
    { symbol: 'SPX500', name: 'S&P 500', category: 'Indices' },
    { symbol: 'DJ30', name: 'Dow Jones 30', category: 'Indices' },
    { symbol: 'USTEC', name: 'US Tech 100', category: 'Indices' },
    { symbol: 'USDX', name: 'US Dollar Index', category: 'Indices' },
    { symbol: 'VIX', name: 'Volatility Index', category: 'Indices' },
    // Commodities (3)
    { symbol: 'USOIL', name: 'US Oil', category: 'Commodities' },
    { symbol: 'UKOIL', name: 'UK Oil', category: 'Commodities' },
  ...
  const fetchInstruments = async () => {
    // Don't fetch from API - use the default instruments list which has all instruments
    // The API only returns instruments with live prices which may be incomplete
    // Keep using the default instruments initialized in useState
  }

  // Default instruments fallback (matching backend POPULAR_INSTRUMENTS)
  ...
  // Filter instruments by segment (API returns 'category' field)
  const getFilteredInstruments = (segment) => {
    if (!segment) return instruments
    return instruments.filter(inst => inst.category === segment)
  }

  const fetchCharges = async () => {
    setLoading(true)
    try {
      // Fetch ALL charges (no segment filter) to show charges for all segments
  ...
  const getLevelLabel = (charge) => {
    ...
    if (charge.level === 'INSTRUMENT') return `${charge.instrumentSymbol} (${charge.segment || 'All'})`
    if (charge.level === 'SEGMENT') return `${charge.segment} - All Instruments`
    if (charge.level === 'GLOBAL') return 'Global - All Segments'
    return charge.level
  }

  return (
    <AdminLayout title="Trading Charges" subtitle="Manage commission, spread & swap for all segments">
          ...
          <div className="p-4">
            {loading ? (
              ...
              <div className="space-y-2">
                {/* Table Header */}
                <div className="flex items-center px-3 py-2 text-xs text-gray-500 border-b border-gray-700">
                  <span className="w-24 text-center">Level</span>
                  <span className="w-28 text-center">Account Type</span>
                  <span className="w-24 text-center">Segment</span>
                  <span className="w-32 text-center">Instrument</span>
                  <span className="flex-1"></span>
                  <span className="w-32 text-right">Value</span>
                  <span className="w-16"></span>
                </div>
                {charges.filter(c => c.commissionValue > 0).map((charge) => (
                  <div key={charge._id} className="flex items-center p-3 bg-dark-700 rounded-lg">
                    {/* Level Badge */}
                    <span className="w-24 flex justify-center">
                      <span className="px-2 py-0.5 bg-gray-600 text-gray-300 text-xs rounded font-medium">{charge.level}</span>
                    </span>
                    {/* 1. Account Type Badge */}
                    <span className="w-28 flex justify-center">
                      <span className={`px-2 py-0.5 text-xs rounded border truncate max-w-full ${charge.accountTypeId ? 'bg-blue-600/30 text-blue-400 border-blue-500/30' : 'bg-dark-600 text-gray-500 border-gray-600'}`}>
                        {charge.accountTypeId ? accountTypes.find(a => a._id === (charge.accountTypeId?._id || charge.accountTypeId))?.name || 'Account Type' : 'All Accounts'}
                      </span>
                    </span>
                    {/* 2. Segment Badge */}
                    <span className="w-24 flex justify-center">
                      <span className={`px-2 py-0.5 text-xs rounded border ${charge.segment ? 'bg-purple-600/30 text-purple-400 border-purple-500/30' : 'bg-dark-600 text-gray-500 border-gray-600'}`}>
                        {charge.segment || 'All Segments'}
                      </span>
                    </span>
                    {/* 3. Instrument Badge */}
                    <span className="w-32 flex justify-center">
                      <span className={`px-2 py-0.5 text-xs rounded border ${charge.instrumentSymbol ? 'bg-green-600/30 text-green-400 border-green-500/30' : 'bg-dark-600 text-gray-500 border-gray-600'}`}>
                        {charge.instrumentSymbol || 'All Instruments'}
                      </span>
                    </span>
                    {/* User Badge */}
                    <span className="flex-1 flex items-center gap-1">
                      {charge.userId && (
                        <span className="px-2 py-0.5 bg-orange-600/30 text-orange-400 text-xs rounded border border-orange-500/30">
                          {charge.userId?.firstName ? `${charge.userId.firstName} ${charge.userId.lastName || ''}`.trim() : charge.userId?.email || 'User'}
                        </span>
                      )}
                    </span>
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center px-3 py-2 text-xs text-gray-500 border-b border-gray-700">
                  ...
                  <span className="w-32 text-center">Instrument</span>
                  <span className="flex-1"></span>
                  <span className="w-40 text-right">Value</span>
                  <span className="w-16"></span>
                </div>
          ...
          <div className="bg-dark-800 rounded-xl w-full max-w-lg border border-gray-700 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <select value={form.segment} onChange={(e) => setForm({ ...form, segment: e.target.value, instrumentSymbol: '', level: form.accountTypeId ? 'ACCOUNT_TYPE' : (e.target.value ? 'SEGMENT' : form.level) })} className="w-full px-3 py-2 bg-dark-700 border border-gray-600 rounded-lg text-white text-sm">
              ...
              <div>
                ...
                <select value={form.instrumentSymbol} onChange={(e) => setForm({ ...form, instrumentSymbol: e.target.value, level: e.target.value ? 'INSTRUMENT' : (form.accountTypeId ? 'ACCOUNT_TYPE' : (form.segment ? 'SEGMENT' : 'GLOBAL')) })} className="w-full px-3 py-2 bg-dark-700 border border-gray-600 rounded-lg text-white text-sm">
                  ...
                  {/* Dynamic instruments from API filtered by segment */}
                  {form.segment ? (
                    getFilteredInstruments(form.segment).map(inst => (
                      <option key={inst.symbol} value={inst.symbol}>{inst.symbol} ({inst.name})</option>
                    ))
                  ) : (
                    ['Forex', 'Metals', 'Crypto', 'Indices', 'Commodities'].map(seg => {
                      const segInstruments = getFilteredInstruments(seg)
                      if (segInstruments.length === 0) return null
                      return (
                        <optgroup key={seg} label={seg}>
                          {segInstruments.map(inst => (
                            <option key={inst.symbol} value={inst.symbol}>{inst.symbol} ({inst.name})</option>
                          ))}
                        </optgroup>
                      )
