...
export default function StarterFundPage() {
  const planDetails = [
    { label: 'Funding Amount', value: '$5,000', icon: DollarSign },
    ...
    { label: 'Leverage', value: '1:50', icon: Gauge },
  ];
    ...
    <>
      ...
      <section className="pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <p className="text-white/60 mb-8">Best for beginner traders starting funded journey.</p>
          <button className="btn-primary px-8 py-4 text-lg bg-gradient-to-r from-emerald-500 to-emerald-600">
            Apply Now
          </button>
        </div>
      </section>
            ...
            {planDetails.map((detail) => (
              <div key={detail.label} className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center">
                <div className="w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center mx-auto mb-3">
                  <detail.icon className="w-6 h-6 text-emerald-400" />
                </div>
      ...
      {/* CTA Section */}
      <section className="py-20 bg-gradient-to-r from-emerald-500/20 to-gold/10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-display text-3xl sm:text-4xl font-semibold text-white mb-4">
            Start your funded journey today.
          </h2>
          <p className="text-white/70 mb-8">Get access to $5,000 in trading capital with up to 80% profit split.</p>
          <button className="btn-primary px-10 py-4 text-lg bg-gradient-to-r from-emerald-500 to-emerald-600">
            Get Funded
          </button>
        </div>
      </section>
